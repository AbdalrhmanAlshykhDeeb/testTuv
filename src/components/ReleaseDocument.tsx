...
const ReleaseDocument = forwardRef<HTMLDivElement, any>(({}, ref) => {
  const editMode = true;
  ...
  return (
    <div className="mt-14  mx-auto px-6" ref={ref}>
      <header className="flex">
        ...
        <div className="basis-4/12">
          <h1 className="text-center leading-none font-bold text-xl">
            <span className="block pb-5">نموذج رقم (٣)</span>
          </h1>
      ...
      <div className="content pb-6 border-2 border-black">
        ...
        <div className="border-t-2 px-2 text-sm print:text-xs font-bold border-black flex justify-between">
          <span className="block en">
            Visually check the packages /goods after request from Customs
          </span>
          <span className="block ar">
            فحص بصري للحزم / البضائع بعد الطلب من الكمارك
          </span>
        </div>

        <div className="flex justify-between border-t-2 border-black p-2 pb-4 text-md print:text-xs font-bold ">
          <ul className="">
            {listEn.map((item, index) => (
              <li key={`list-pdf-${index}`} className="mb-2">
                {item}
              </li>
            ))}
          </ul>
